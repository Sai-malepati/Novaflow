...
interface DataTableProps {
  ...
  // ⭐ NEW – for CreateTaskList only
  onAssigneeClick?: (row: Row) => void
  showAssigneeEditIcon?: boolean
}
...
const DataTable: React.FC<DataTableProps> = ({
  ...
  const getStatusColor = (status: string) => {
    ...
    if (status.includes("New")) return "#4CAF50"
    ...
    if (status.includes("Completed")) return "#FF9800"
                    ...
                    .map((row, i) => (
                      <TableRow key={i}>
                        {columns.map((column) => {
                          ...
                          // Select column – show checkbox
                          if (column.id === "select") {
                            return (
                              <TableCell
                                key={column.id}
                                sx={{ padding: "2rem", lineHeight: "1 !important" }}
                              >
                                <Checkbox size="small" />
                              </TableCell>
                            )
                          ...
                          // // ⭐ Assignee cell – edit icon ONLY when showAssigneeEditIcon = true
                          ...
                          //             onClick={(e) => {
                          //               e.preventDefault()
                          //               if (showAssigneeEditIcon && onAssigneeClick) {
                          //                 onAssigneeClick(row)
                          //               }
                          //             }}
                          //           >
                          ...
                          //           {showAssigneeEditIcon && onAssigneeClick && (
                          //             <IconButton
                          //               size="small"
                          //               sx={{ padding: 0, ml: 0.5 }}
                          //               onClick={() => onAssigneeClick(row)}
                          //             >
                          ...
                          // ⭐ Assignee cell
                          if (column.id === "assignee") {
                            const isEditable = showAssigneeEditIcon && !!onAssigneeClick
                            ...
                            return (
                              ...
                              >
                                {!value ? null : isEditable ? (
                                  // ===== CreateTaskList behaviour: clickable name + edit icon =====
                                  <Box sx={{ display: "flex", alignItems: "center", gap: 0.5 }}>
                                    <MUILink
                                      component="button"
                                      underline="always"
                                  ...
                                  // ===== Default behaviour (Dashboard, TMin, Hit&Leak): plain text =====
                                  <Typography
                                    component="span"
                                    sx={{
                                      fontSize: "0.9rem",
                                      fontWeight: 500,
                                      color: "#5A5A5A", // or remove to use default table text color
                                    }}
                                  >
                                    {value}
                                  </Typography>
